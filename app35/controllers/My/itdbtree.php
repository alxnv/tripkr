...
// выводит на экран все дерево
        ...
        $i++) {
            $arr=$aro[$topid][$i];
            $n=$arr->uid;
            echo '<li><a href="'.my7::baseurl().$arr->name.'/index/id/'.$n.'/lv/'.$ct.'" target="right">'.
                    my7::nbsh(ltrim($arr->naim=='' ? '---' : $arr->naim)).
                    '</a></li>';
    ...
    // удаляет записи в $this->tbl, $tbl2 с идентификатором $id, а также все их подразделы
        ...
        array_push($arr,$id);
        $s=join(',',$arr);
        my7::db()->delete($this->tbl,"uid in ($s)");
        my7::db()->delete($tbl2,"uid in ($s)");
    }
    ...
    function allchildren($id) {
        ...
        do {
            $s=join(',',$ar4);
            $ar2=my7::qlist("select uid from $this->tbl where topid in ($s)");
            $ar4=array();
...
//      where a.idtree=$this->idtree and a.actid=b.uid order by a.topid,a.ordr");
        $ar2=my7::qlist("select a.*,b.name from $this->tbl a,$this->tblact b
                where a.idtree=$this->idtree and a.actid=b.uid order by a.topid,a.ordr");
        $aro=array();
        for ($i=0;
        $i<count($ar2);
...
//  если==0, то одну эту запись
        if (!$this->nolock) my7::qdirect("lock tables $this->tbl write");
        if (is_null($idtree)) {
            $row=my7::qobj("select idtree,topid,ordr from $this->tbl where uid=$id");
            if ($row===false) return;
            $idtree=$row->idtree;
            $topid=$row->topid;
            $ordr=$row->ordr;
        ...
        $arr = my7::qlist("select uid from $this->tbl where idtree=$idtree
...
//my7::qdirect("update $this->tbl set ordr=ordr-1 where");
        ...
        if (!$this->nolock) my7::qdirect('unlock tables');
...
// возвращает идентификатор добавленной записи
        $db=my7::db();
        if (!$this->nolock) my7::qdirect("lock tables $this->tbl write");
        $row=my7::qobj("select max(ordr) as mo from $this->tbl
...
// перемещает запись внутри topid
        ...
        if ($row!==false) {
            ...
            $db=my7::db();
            $id1=$row->ordr;
            $row2=my7::qobj("select max(ordr) as mo from $this->tbl where
...
//$arr2=array();
            $arr = my7::qlist("select uid from $this->tbl
