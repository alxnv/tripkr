...
$error = '';
//$error.=$filename;
$full_path = $file_new_name.my3::alphatrans($file_name);
$ext=my3::getExtension($full_path);
while (is_file($full_path)) {
    $fn=my3::withoutExt($full_path);
    $ext=my3::getExtension($full_path);
    $full_path=$fn.'1.'.$ext;
//    $error.='jj '.$full_path;
}
$http_path = my3::baseUrl().'images/upload/'.basename($full_path); // адрес изображения для обращения через http
//my3::log('ss',$file_name.'@1@'.$ext.'@2@'.in_array(strtolower($ext),$allowed_exts).'@3@'.$file_name_tmp.'@4@'.$full_path);
...
//$error.=' yes ';    
    ...
    $fn1=$full_path;
    @chmod($fn1,0666);
    $s=my3::getExtension($fn1);
    if (strtolower($s)=='jpg' || strtolower($s)=='jpeg') my3::resamplejpeg3(700,0,$fn1,$fn1);
